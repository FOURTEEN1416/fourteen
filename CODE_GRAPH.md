...
| Python 测试用例 | **1247 passed + 4 skipped**（收集 1251） | 2026-09-20 系统 Python 3.12 实跑（四块分块合计约 211s；较上批 +165 = 本批角色卡扩充 +34【16 新卡 × persona 参数化 2 用例 + 交错合并回归 2】+ 09-19 晚通道隔离等先前提交增量约 131 未同步文档）。⚠️ **单进程整跑会在随机位置停住**（非用例失败，属聚合态资源问题），分块跑法见 `AGENTS.md` §4.3 口径注记 |
...
| 测试用例合计 | **1345 个**（1247 Python 通过 + 98 前端通过） | pytest + vitest 实跑 2026-09-20。⚠️ 旧口径 1117（1042 Python + 75 前端，2026-09-01 .venv 实测）随 09-14 主仓事故丢失环境后**已作废**，不再作为可复现基线 |
