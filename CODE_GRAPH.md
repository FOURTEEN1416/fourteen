# 代码图谱 — unique-you (唯一的你) v3.8.18

> 由 维护者 手动维护 | 最后核实: 2026-09-21（**v3.8.18 v1.35 收仓口径统一 + shisi.yaml 行数勘误**：§1.1 Python 测试行刷新 **1615→1657 收集 / 1611→1653 通过 / 4 跳过**（分块 **341 + 387+3 + 494+1 + 431**，0 失败；+42 = 并行窗收编 `test_attribution_isolation_state` / `test_code_review_r2_state_and_llm` / `test_session_key_owner`）；测试合计 **1709→1751**（1653 Py + 98 FE）；**勘误** `config/shisi.yaml` 实为 **91 行**（65 有效行），v1.35 头部「41 行」系 voice 段恢复前中间态误记；端点 **220/186** 不变；vitest **98/98** 复跑。同步 AGENTS §0/§2/§4.3、README 徽章/口径/结构树、LOG 勘误两处。）
...
| Python 测试用例 | **1653 passed + 4 skipped**（收集 **1657**） | 2026-09-21 v1.35 收仓口径统一·分块实跑（341 + 387+3 + 494+1 + 431，与 `--collect-only` 吻合；0 失败；+42 = 并行窗收编 3 测试文件）。⚠️ **基线随 `config/characters/` 卡数浮动**：该目录被 gitignore（不入公开仓），`test_persona_injection` 的用例数 = **2 × 卡数 + 7**。**引用基线必须同时声明卡数**（本次 41 张在位） |
...
| 测试用例合计 | **1751 个**（1653 Python 通过 + 98 前端通过） | pytest + vitest 实跑 2026-09-21 v1.35 收仓口径统一（⚠️ Python 侧跳过 4 不计入通过数） |
