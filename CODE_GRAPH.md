...
| 日期 | 提交 | 变更摘要 |
|------|------|---------|
| 2026-09-17 | 10c8f0f + 5e4ecb5 | **v3.7.0 人设/主动消息三连修（生产日志实证驱动）**：(1) `character_routes.py` activate 带 JWT 时同步当前登录用户全部 `wechat_bindings`（复用 `upsert_binding` 刷新运行中进程缓存，web 切角色→微信实时生效）——修复 web"设为活跃"与微信人设真源断裂（旧 `set_user_character("default")` 空转）；(2) 角色卡长锚点截断保留（旧 >20 字整条丢弃，绑定卡最丰富锚点被过滤）；(3) emoji 五处提示词语义化（默认=每条最多一个、仅情绪强烈时用）；(4) `scheduler._deliver()` asyncio.run 替代非主线程必炸的 get_event_loop（生产 64 触发 0 送达→修复后 1/1 送达）；`_check_ase` 回退 ASE 自身 `_hours_since_last_chat()`（旧回退≈5min 导致 missing_bonus 恒 0）；发送目标改 `get_bound_wxids()` 定向；(5) `MultiProviderGateway` 补 `chat_sync`（ASE LLM 生成静默回落模板的根因）；(6) update/activate 新增 `_invalidate_knowledge_index`（ensure_index 优先磁盘加载旧索引永不重建）；(7) 删零消费 `shared/Badge.tsx`；(8) 服务器同步 24 张唯一卡（53 张同名去重，保留绑定卡 62105bca）；(9) 测试 +13（test_persona_binding_fixes.py），1028 Python + 87 前端全绿；部署闭环 5e4ecb5+health 200，"微信主动发送成功/主动消息已投递: wechat"实证 |
