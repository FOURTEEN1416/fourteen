...
| Python 测试用例 | **1082 passed + 4 skipped**（收集 1086） | 2026-09-19 系统 Python 3.12 实跑（分块合计 164.2s；较上批 +22 = 主动消息「配额/投递解耦」批次新增回归用例，全在 `tests/test_proactive.py`）。⚠️ **单进程整跑会在随机位置停住**（非用例失败，属聚合态资源问题），分块跑法见 `AGENTS.md` §4.3 口径注记 |
| 前端测试用例 | **87 个全部通过 / 15 文件** | 2026-09-18 `npm test`（vitest run）+ `tsc --noEmit` 0 错误 |
| 测试用例合计 | **1169 个**（1082 Python 通过 + 87 前端通过） | pytest + vitest 实跑 2026-09-19。⚠️ 旧口径 1117（1042 Python + 75 前端，2026-09-01 .venv 实测）随 09-14 主仓事故丢失环境后**已作废**，不再作为可复现基线 |
