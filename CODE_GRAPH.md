...
| Python 测试用例 | **1319 passed + 4 skipped**（收集 1323） | 2026-09-20 系统 Python 3.12 分块实跑（241+386+328+364，与 `--collect-only` 1323 精确吻合；`test_knowledge_routes_index.py` 已在分块清单内，无需再单独跑）。⚠️ **单进程整跑会在随机位置停住**（非用例失败，属聚合态资源问题），分块跑法见 `AGENTS.md` §4.3 口径注记 |
