...
- **技术栈**：Python 3.10+ / React 19 / Vite 8 / TypeScript 6 / Tailwind 4 / Zustand 5 / 测试 **1526**（**1420** Py 通过 + 4 跳过，98 FE 通过；2026-09-20 主检出含 41 卡分块实测，收集 **1429**；无卡 worktree/CI 约 1344，见 §4.3）/ MIT
- **核心能力**（见 `README.md`）：
...
| 安全 | 安全开发 | `security/` |
| 测试 | QA | `tests/`（**1424** Python 通过 + 4 跳过；前端 **98**；主检出 2026-09-20 分块实测收集 **1429**） |
...
- 626+ 测试已建立，新功能必须附测试（实测基线：**1424** Python 通过 + 4 跳过 / 98 前端；主检出含 41 卡收集 **1429**，2026-09-20 分块实跑）
- 用户明确反对采样验证，要求完整验证
- **测试口径注记（2026-09-20 十次刷新）**：系统 Python 3.12 实测 **1414 收集 / 1410 通过 / 4 跳过**（十次刷新：包 Q A/B/C 收编 +63（含 `tests/test_abc_*.py`×5）；主检出分块 **396 +323 +373+3 +318+1** 精确吻合）。（上批九次刷新 **1351 收集 / 1347 通过**）。（上批八次刷新 **1328 收集 / 1324 通过**）。（上批七次刷新 **1323 收集 / 1319 通过**）。（上批六次刷新 **1307 收集 / 1303 通过**）+ 前端 vitest **98/98**（16 文件）。**角色卡现役 41 张**（25 既有 + 16 文学导入），persona 注入参数化用例数 = 2 × 卡数，卡数变动会动基数。
  ...
  ```
  四块结果相加应等于 `--collect-only` 的收集数（当前主检出 **1429** ≈ **330+1 +398+4 +321 +289+5（无卡口径）/ 主检出含卡约 +80 persona** 精确吻合，2026-09-20 全仓复核实跑；无卡 worktree 约 1344。`test_knowledge_routes_index.py` 已在分块清单内）。
...
| 版本 | 日期 | 变更 |
|------|------|------|
| **v1.26** | **2026-09-20** | **全仓复核**：记忆会话隔离补漏（fact 抽取/memory_context/retrieve facts 降级）+ storyline UTC 时间戳 + 文档口径 1424/1420/4 + 215/181 复测 |
