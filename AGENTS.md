...
- **技术栈**：Python 3.10+ / React 19 / Vite 8 / TypeScript 6 / Tailwind 4 / Zustand 5 / 测试 1367（1269 Py 通过 + 4 跳过，98 FE 通过；2026-09-20 系统 Python 3.12 + vitest 实测，见 §4.3）/ MIT
- **核心能力**（见 `README.md`）：
...
| 安全 | 安全开发 | `security/` |
| 测试 | QA | `tests/`（1269 Python 测试通过 + 4 跳过；前端 98 测试；2026-09-20 实测，口径同 CODE_GRAPH §1.1) |
...
- 626+ 测试已建立，新功能必须附测试（实测基线:1269 Python 通过 + 4 跳过 / 98 前端，2026-09-20 实跑,与 CODE_GRAPH §1.1 对齐)
- 用户明确反对采样验证，要求完整验证
- **测试口径注记（2026-09-20 五次刷新）**：系统 Python 3.12 实测 **1273 收集 / 1269 通过 / 4 跳过**（五次刷新：本批 +14 回复质量根治回归 test_reply_quality_overhaul）（较上批 +168 = 本批角色卡扩充 +34【16 新卡 × persona 参数化 2 用例】+ 知识库修复回归 5【交错合并 2 + knowledge 路由建索引 3】+ 09-19 晚通道隔离等先前提交增量约 131 未同步文档；分块实跑合计约 235s）+ 前端 vitest **98/98**（16 文件）。**角色卡现役 41 张**（25 既有 + 16 文学导入），persona 注入参数化用例数 = 2 × 卡数，卡数变动会动基数。
  ...
  ```
  四块结果相加应等于 `--collect-only` 的收集数（当前 1273；2026-09-20 复测四块 377+369+318+205 精确吻合，`test_knowledge_routes_index.py` 已在分块清单内，无需再单独跑）。
