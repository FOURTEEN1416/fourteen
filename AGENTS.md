...
- **技术栈**：Python 3.10+ / React 19 / Vite 8 / TypeScript 6 / Tailwind 4 / Zustand 5 / 测试 **1532**（**1444** Py 通过 + 4 跳过，98 FE 通过；2026-09-21 隔离根治分块实测，收集 **1448**；⚠️ **随 `config/characters/` 卡数浮动**（现役 41 张），见 §4.3）/ MIT
- **核心能力**（见 `README.md`）：
...
| 安全 | 安全开发 | `security/` |
| 测试 | QA | `tests/`（**1444** Python 通过 + 4 跳过；前端 **98**；2026-09-21 隔离根治分块实测收集 **1448**；⚠️ 随 `config/characters/` 卡数浮动，现役 41 张） |
...
- 626+ 测试已建立，新功能必须附测试（实测基线：**1444** Python 通过 + 4 跳过 / 98 前端；2026-09-21 隔离根治分块实测收集 **1448**，0 失败；现役角色卡 **41 张**）
- 用户明确反对采样验证，要求完整验证
- **测试口径注记（2026-09-21 十二次刷新）**：系统 Python 3.12 实测 **1448 收集 / 1444 通过 / 4 跳过 / 0 失败**（十二次刷新：用户隔离 P0 +`tests/test_user_isolation_chain.py` 12 例 + 既有隔离语义改钉；主检出分块 **377+3 + 351+1 + 360 + 356** 精确吻合；角色卡 **41 张**在位）。（上批十一次刷新 **1436/1432/4**）。（上批十次刷新 **1414/1410/4**）。
- **测试口径注记（2026-09-20 十一次刷新）**：系统 Python 3.12 实测 **1436 收集 / 1432 通过 / 4 跳过 / 0 失败**（十一次刷新：全仓历遍批次 +7 用例 = `tests/test_async_bridge_contract.py` 5 + 信封位次 1 + 批量 GC 真执行 1；主检出分块 **314 + 384+3 + 330+1 + 404** 精确吻合）。（上批十次刷新 **1414 收集 / 1410 通过**）。（上批九次刷新 **1351 收集 / 1347 通过**）。（上批八次刷新 **1328 收集 / 1324 通过**）。（上批七次刷新 **1323 收集 / 1319 通过**）。（上批六次刷新 **1307 收集 / 1303 通过**）+ 前端 vitest **98/98**（16 文件）。
  ...
  ```
  四块结果相加应等于 `--collect-only` 的收集数（当前检出 **1448** = **377+3 + 351+1 + 360 + 356** 精确吻合，2026-09-21 隔离根治分块实跑；收集数随 `config/characters/` 卡数浮动 = 2 × 卡数 + 7，**现役 41 张**。`test_knowledge_routes_index.py` 已在分块清单内）。
