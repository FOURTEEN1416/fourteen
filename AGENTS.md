...
- **技术栈**：Python 3.10+ / React 19 / Vite 8 / TypeScript 6 / Tailwind 4 / Zustand 5 / 测试 1426（1328 Py 通过 + 4 跳过，98 FE 通过；2026-09-20 系统 Python 3.12 + vitest 实测，见 §4.3）/ MIT
- **核心能力**（见 `README.md`）：
...
| 安全 | 安全开发 | `security/` |
| 测试 | QA | `tests/`（1328 Python 测试通过 + 4 跳过；前端 98 测试；2026-09-20 实测，口径同 CODE_GRAPH §1.1) |
...
- **测试口径注记（2026-09-20 九次刷新）**：系统 Python 3.12 实测 **1332 收集 / 1328 通过 / 4 跳过**（九次刷新：本批收口并行窗口在制品 +4 = FrequencyController 状态往返/本地日界 2 + analytics 本地日键 1 + after_chat 错误占位不入库 1；分块实跑 **362+3 +354 +350+1 +262** 与收集精确吻合）。（上批八次刷新 **1328 收集 / 1324 通过**：复核补漏 +5 = local_day_utc_bounds 换算契约 3 + StructuredMemory 本地日窗口真实 SQL 边界 2；分块 241+386+328+369）。（上批七次刷新 **1323 收集 / 1319 通过**：+16 墙钟时区修复回归 —— `tests/test_local_time.py` 13 + `tests/test_memory_pipeline.py` 3；分块 241+386+328+364）（上批六次刷新 **1307 收集 / 1303 通过**：+34 提醒意图管线回归）（上批五次刷新 +14 回复质量根治回归）（更早 +168 = 角色卡扩充 +34 + 知识库修复回归 5 + 通道隔离等增量）+ 前端 vitest **98/98**（16 文件）。**角色卡现役 41 张**（25 既有 + 16 文学导入），persona 注入参数化用例数 = 2 × 卡数，卡数变动会动基数。
  ...
  ```
  四块结果相加应等于 `--collect-only` 的收集数（当前 **1332**；2026-09-20 复测四块 362+3 +354 +350+1 +262 精确吻合，`test_knowledge_routes_index.py` 已在分块清单内，无需再单独跑）。
