...
- **技术栈**：Python 3.10+ / React 19 / Vite 8 / TypeScript 6 / Tailwind 4 / Zustand 5 / 测试 **1991**（**1893** Py 通过 + 1 跳过，98 FE 通过；2026-09-22 R3 验收收口实测：本地=服务器双端收集 **1894** / 0 失败；角色卡 **41 张**）/ MIT
- **核心能力**（见 `README.md`）：
...
| 安全 | 安全开发 | `security/` |
| 测试 | QA | `tests/`（**1893** Python 通过 + 1 跳过；前端 **98**；2026-09-22 R3 验收收口双端实测收集 **1894** / 0 失败；现役角色卡 41 张） |
...
- 626+ 测试已建立，新功能必须附测试（实测基线：**1688** Python 通过 + 1 跳过 / 98 前端；2026-09-21 selftalk 四项移植后分块实测收集 **1689**，0 失败；现役角色卡 **41 张**）
- 用户明确反对采样验证，要求完整验证
