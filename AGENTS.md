...
> **版本**：v1.37（2026-09-22 **六域二次全面排查根治：人设/记忆/情感/主动/提醒/隔离·五块分块验收**）——用户指令「继续全面排查六域」→ 第二轮排查 **6 P0 / 9 P1 / 10 P2** → 裁决「按推荐全面进行」，沿袭分块规程（修一块查一块验收一块，禁补丁式修复与备注式删除）。**块A/P0**（`04c7ed2`）：情绪衰减基准落盘 + 缺失即回填——首版修复（同日 `9a54fca`）以 `None` 作哨兵，而 `_hours_since_last_emotion_decay()` 对 None 返 0.0、`if hours > 0` 永假、推进基准的赋值又在 if 内 → 基准永远停在 None，衰减换了一种死法依然从未发生（生产 00:05 日志 hours=0.01 实证）；提醒时刻校验改走本地墙钟（UTC 判「过去」在 +0800 下有 8 小时误判窗）。**块B/P0**（`3ec093f`）：投递异常不再被日志通道掩盖（异常既写日志又当返回值用）；`main.py` 入口 websocket 投递改定向。**块C/P0**（`af5c41e`）：**出站消息归属全链贯通**——主动消息/提醒/微信追问写历史只落 `role+session_id`，assistant 行无 `character_id` → 切角色后新角色继承他人台词；`record_outbound_message` 签名补齐归属、`memory_pipeline` 写五归属（character_id/user_key/turn_id/importance/channel）、三调用点各自解析，新建 `utils/character_resolver` 为「会话键→角色」唯一 owner；**存量空归属懒回填**（下游过滤含 `OR character_id=''`，存量全空时该条件恒真 = 过滤整体失效，生产实测 88% 空归属）→ 两段式「SQL 保留空归属 → 幂等 UPDATE 回填 + 补 user_key → 再过滤」；另修 `run_api._character_resolver` 的 `"default"` truthy 缺陷。**块D/P1**（`c37d355`）：`dry_run` **死路径根治**（闸门在生成前 `return` 使 `generate_with_llm` 整条不可达、urgency 恒 None）；决策层接地补 `last_user_message`/`response_rate`；`note_user_interaction` 就近落盘。**块E/P1+P2**（`04229eb`，14 文件 456+/67-）：⑩ **节流账本落盘**（`_llm_proactive_next_ok`/`_deliver_fail_counts`/`_disabled_event_day`/`_important_dates_sent` 由纯内存落 `scheduler_config.json` 的 `throttle` 段，逐字段容错 + NaN/inf/负值钳除 + 上限 200；旧实现重启归零 → 退避永停第 1 档、等待窗作废、每日去重失效、祝福重发）；⑪ `ase_engine.save_state` 走 `json_state.atomic_write_json`；⑫ **好感度双真源根治（本轮最重）**——`mapper.sync` 收**会话键**而 `user_scheduler._persist_affinity` 收**裸 user_id**，两套键空间按构造零交集（生产实证：审计 129 行 5 键 vs 点存 3 键无一重合），enhancer 重启回放拿不到任何可回放数据；统一 `user_key_from_session` + **双写**（新增 `default_db_path()` 共享 owner）+ 点存兜底回填 + 恢复路径独立降级；⑬ **`semantic_memory` 向量召回读侧协程缺陷**——`vector_memory._search` 是 `async def` 却被同步直调，返回 coroutine 且 `or []` 不触发 → **向量召回恒空**（写侧已修读侧漏修，「谎言家族」新例）；改 `inspect.isawaitable` + `run_async` + `where` 降级；⑭ 反思按会话检索空归属死结（下推 WHERE 使空归属历史连候选都进不来 → 取回后 Python 过滤 + 丢弃计数，宁缺毋串）；⑮ `get_cross_session_tail` 排序改 `id DESC`（秒级时间戳禁判序）；⑯ `time_awareness_tool` 默认日期改 `now_local().date()`；⑰ 删死参 `persona_service.build_system_prompt(character_overrides=...)`；⑱ **激活角色缓存击穿数据源身份**（缓存键只有目录指纹，`_list_all_characters` 被替换时目录未变 → 返回陈旧 active id；补 `id(_list_all_characters)`）。**🔴 自伤事故与根治**：块E 加落盘后**未隔离 `_CONFIG_PATH` 的既有夹具把测试键写进开发机真实 `data/scheduler_config.json`**（gitignored → 不被 git 发现却反向改变后续用例行为，首跑 10 处失败）→ `tests/conftest.py` 增 autouse 夹具 `isolate_runtime_state_files`（4 类状态文件重定向到 per-test 临时目录）。**验证**：本地分块 **1839 收集 / 1838 通过 / 1 跳过 / 0 失败** + ruff 全仓 0 错 + `scripts/ci_gates.py` **4/4 门禁**；**基线对照**（`git stash` 同环境同分块）**21 失败 / 1817 通过 → 0 失败**；**突变验红 5/5** 命中（协程直调回退 2 例 / 账本不落盘 3 例 / 删点存兜底 1 例 / 反思退回严格 WHERE 首轮未命中→补 2 例后转红 / 缓存键去数据源身份 2 例），还原后 `MUTATED` 残留 0；新增 `tests/test_block_e_hardening.py` 27 例。**遗留**：生产 affinity 双键空间历史数据不迁移、`affinity_records` 测试遗留键未清、本批未部署（归用户裁决）、conftest 隔离为进程级兜底。
> **版本**：v1.36（2026-09-22 **六域全面排查根治：人设/记忆/情感/主动/提醒/隔离**）——用户指令「全面排查六域」→「按推荐全面修复，所有部分都需要修……修一块查一块验收一块……不接受补丁式的修复，备注式的删除，激进一点，彻底一点」，四块分块落地。**块1/P0**（`9a54fca`）：① 遗忘模型时间输入根治——`user_facts.updated_at` 是 SQLite `CURRENT_TIMESTAMP` 的 naive UTC 串，旧实现与 aware 相减恒抛 TypeError 被吞成 `days_old=30.0` 恒值 → 按龄遗忘整体失效（低置信度事实每晚误删、高置信度永不遗忘）；新 `_fact_age_days` 纯函数（naive 按 UTC 解释、解析失败/缺失钳 0 fail-safe）。② `set_reminder` trigger_time 三关校验（格式+日历有效性+过去时刻 5 分钟容忍窗，归一秒级串与轮询同构）——畸形串「明早六点」按字典序永不到期=「说了会叫却没叫」残留通道。③ 情绪时间衰减基准独立记账 `_hours_since_last_emotion_decay`（旧误用距 ASE tick ≈5 分钟，每日衰减形同关闭）；失去读者的 `_last_check_time` 整删。**块2/P1**（`f6a4637`+`1ba015a`）：④ AffinityEnhancer 重启归零根治——从自写 `affinity_records` 审计日志回放每键最新值（含 `_last_interaction` 转 aware，否则 DecayEngine 相减崩）；`setup_shisi` 的 db_path 接线到 enhancer（旧恒指宿主真库，集成测试 affinity 100≠50 实锤）。⑤ 重要日期祝福多用户化——旧实现全局广播给所有 owner 全部绑定 peer、dedup 无用户维度（A 发过同日生日的 B 被吞）、不读画像生日；现两路检查（画像生日 `parse_birthday_hint` 公历解析、农历宁缺毋错 + 全局日期逐用户定向）。**块3/P2**（`8d3acf9`）：⑥ 事实检索 user_key 过滤下推 SQL/Chroma where（旧先全库 LIMIT 20 后 Python 过滤，他人事实占满窗口时本人静默漏检）。⑦ **pending_events 死链整拆**：CrossSessionReasoner `git rm` + 4 处接线全删 + 幂等 DROP 表（提取→存储→检索后被 orchestrator pop 丢弃、resolve 零调用、表无界增长，DELETION_LOG 2026-09-22 节）。⑧ `event_ledger.prune` 增 `preserve_types`（画像事件=画像唯一写权威永久豁免，不再套 30 天 TTL）；`project_profile` 窗口改取最新 limit 条（旧取最早，事件超限画像卡死旧值）。**块4/P3**（`11bdc57`）：⑨ DB 版 WorkingMemory 整删（working_memory/sessions 两表唯一写入者、零生产零测试消费者）；死表批量出库+幂等 DROP（working_memory/sessions/affinity_log/emotion_trajectory/pending_events）+ affinity_log CRUD 与 `StructuredMemory.get_stats` 零消费者删除；vector_memory 集合表移除 emotion_trajectory。⑩ **web 侧投递从无归属广播收口为按会话键定向**：WebSocketServer 连接→会话映射 + `send_proactive_to_session`（0 送达判失败）；提醒 ws_sender 签名改 `(session_key, text)`；scheduler web 分支不再回退广播；提醒文案角色名按会话解析（character_resolver）；`query_reminders` 无 `_meta` 拒绝执行。**验证**：本地分块 **1750 收集 / 1749 通过 / 1 跳过 / 0 失败**（461+382+553+1+353 精确吻合）+ vitest 98/98 + ruff 全仓 0 错；端点重测 **APIRoute 220 / 唯一路径 186**（openapi.json 实测，零端点变更）；服务器 Fast-forward `f699a8e→1ba015a`、health 200、服务 active、git blob 哈希 5/5 三端一致（md5 直接比对被 CRLF 骗——沿用归一化口径）、服务器分块全量 **1750 收集 / 1749 通过 / 1 跳过 / 0 失败**（465+362+544+1+378）+ 六域冒烟 196 通过。**并发登记**：并行窗口 `86af331`（身份拷问剧本）+`f699a8e`（docs 落账）与本案四块在 main 串行交错提交，无文件冲突，全量回归覆盖其改动。突变验红 **8/8** 命中（遗忘相减回退 2 例红/提醒透传 9 例红/衰减恒 0.08h 红/enhancer 禁用恢复 5 例红/dedup 去 user_key 红/prune 去豁免红/投影回 ASC 红/search_facts 去 key_sql 红——首版夹具被 near-dup 合并塞不满窗口的假验红已修正重做）。
> **版本**：v1.35（2026-09-21 **备注式删除与逻辑矛盾收口 + 多窗口收仓**）——用户指令「扫描仓库中的备注式删除，逻辑矛盾的代码设计」→「按推荐进行、激进一次性根治、严禁备注式清除」→「另一窗口已停止，全权交给你，进行最后的复核和收仓」。主线：① **配置真源收口**：`config/shisi.yaml` 234→**91 行**（65 有效行；勘误：原记 41 系 voice 段恢复前中间态读数；仅存 `memory` 5 键 + `affinity` 4 键 + `voice` 段），`emotion.yaml` 删 4 段，`system.yaml` 删 20 死键（含 3 处与顶层同名的 fusion 双真源）；**消除 `shisi.yaml security.*` 与 `system.yaml safety.*` 双套安全配置**（改前者静默无效）。② **4 个整模块删除**：`observability/tracing.py`（`tracer.span()` 全仓 0 调用、`start_trace()` 0 调用 → `get_trace_id()` 恒空的**假可观测性**）、`my_character/style_enhancer.py`、`shisi/core/services/emotion_detector.py`（仅墓碑 re-export，其「好烦」净 +0.5 的内在矛盾随之消除）、`shisi/wechat/proactive_messenger.py`（registry 实例化但 5 方法零生产调用，同 09-17 未接线微信能力裁决例）；另删死函数 `path_security.safe_join_path`（含 startswith 前缀绕过缺陷）/ `wechat_connector._clear_credentials`（已被 per-connector 凭证自愈取代）/ 孤儿模型 `MemoryFactResponse`/`StorylineDetectResponse` / 6 个 `*V2``*Optimized` 兼容别名。③ **多窗口收仓**：`37a72ab`（死码收口 25 文件 +115/−863）+ `c8dc2b1`（并行窗口在制品 34 文件 +2499/−444，含 `utils/session_key`/`llm_bridge`/`emotion_state`/`json_state` 与 `shisi/core/conversation_turn` 三大 owner 唯一化）。**验证**：ruff 0.16.8 全仓 0 错；分块 **1657 收集 / 1653 通过 / 4 跳过 / 0 失败**（**341 + 387+3 + 494+1 + 431** 精确吻合；当日首跑汇总行曾被环境钩子吞掉，同日复跑未复现，以复跑精确计数为准）；端点重测 **APIRoute 220 / 唯一路径 186 / len(app.routes)=224**（方法 105 GET / 79 POST / 16 PUT / 20 DELETE，与 v1.34 一致）；三份 yaml 解析通过。**收仓细节**：3 个 worktree（`wt/agent-x`/`wt/ax-review`/`wt/selftalk-fix`）已注销并移除目录、**分支全部保留**；🔴 `wt/selftalk-fix` **不可 merge** —— fork 基点早于项2–10 死码清除，A 类 27 文件全是已删死码，merge 会复活死码并把 132 文件回退到旧版。**遗留**：`peer-projects`（~250M，可 clone 重建）与 `产物隔离_小凌研究`（393M，**不可重建**）按裁决保留。**A 档已三端同步（同日补记二）**：`78b6a47` 已 push 并部署——服务器 `8d87134e→78b6a478` Fast-forward（缺口 22 提交/125 文件），health 200、服务 active、md5 抽验 4/4 一致（详见 LOG v1.35 补记二）。
...
- **技术栈**：Python 3.10+ / React 19 / Vite 8 / TypeScript 6 / Tailwind 4 / Zustand 5 / 测试 **1995**（**1897** Py 通过 + 1 跳过，98 FE 通过；2026-09-23 收尾轮本地四分块实测收集 **1898** / 0 失败；角色卡 **41 张**）/ MIT
- **核心能力**（见 `README.md`）：
...
| 安全 | 安全开发 | `security/` |
| 测试 | QA | `tests/`（**1897** Python 通过 + 1 跳过；前端 **98**；2026-09-23 收尾轮本地四分块实测收集 **1898** / 0 失败；现役角色卡 41 张） |
...
- 626+ 测试已建立，新功能必须附测试（实测基线：**1897** Python 通过 + 1 跳过 / 98 前端；2026-09-23 收尾轮本地四分块实测收集 **1898**，0 失败；现役角色卡 **41 张**）
- 用户明确反对采样验证，要求完整验证
- **测试口径注记（2026-09-23 二十四次刷新）**：上线收口 + 收尾轮（三端 `6f80a5c`）——`ad828f7`+`d57cb5f` 已部署重启（health 200、nginx 零改动、blob 三端一致）；部署当场抓出并根治**启动通道同步 P2**（`migrate_legacy_wechat_channel.py` 同步 `asyncio.run()` 在 lifespan 运行循环内必抛、自 09-21 从未成功，116 行告警→修复后「已同步 3 条」×worker，+1 行为回归红测先行）；服务器账本隔离专项验收 98 例、`agent_plane.db` 1526→1526 零污染（**非全量回归**）。主动消息按默默令**止改**，账本终结分析入 LOG（send 28 全为 API 受理、web 协议无送达回执；skip 396 llm_wait_window）。**收尾轮全量四分块实测 1898 收集 / 1897 通过 / 1 跳过 / 0 失败**（**428 / 577+1跳过 / 411 / 481** 精确吻合，124 文件；41 卡在位、工作树仅文档批；ruff 0 错）。**徽章刷新 1991 → 1995**（1897 Py + 98 FE），§0/§2/§4.3 与 README/CODE_GRAPH v3.8.22 同步。
- **测试口径注记（2026-09-23 二十三次刷新）**：五域可靠性批次（两枚 P1：`ad828f7` FrequencyAdapter minimal 粘滞逐级回升 / `d57cb5f` 账本测试隔离）本窗四分块实测 **1897 收集 / 1896 通过 / 1 跳过 / 0 失败**（**435 / 550+1跳过 / 439 / 472** 精确吻合；41 卡在位、工作树 clean 仅本批文件；ruff 0 错 + ci_gates 4/4；已 push `d57cb5f`，**服务器未上线**）。徽章暂留 1991（=已部署 68851a5 口径），随部署批刷新。⚠️ 新教训：**服务器跑测试会直写生产 `data/agent_plane.db` 与 app.log**（五域读数曾被 14 条测试 memory_write 混入）——conftest 已兜底，服务器侧读数仍须按 session_key 真实度甄别。
- **测试口径注记（2026-09-22 二十二次刷新）**：R3 验收收口（二次上线 `e869a57`+`68851a5`）——本地=服务器双端干净四分块实测 **1894 收集 / 1893 通过 / 1 跳过 / 0 失败**（服务器 **539 / 561+1跳过 / 416 / 377**；本地 **435 / 550+1跳过 / 439 / 469**；41 卡在位、三端 HEAD 一致 `68851a5`、双端工作树 clean）。本批净增 3 例：hub `health_check` 2（`e869a57`，收 `/api/proactive/state` 空引擎 500）+ 门禁安装器解释器绑定 1（`68851a5`，收服务器无 `python` 环境钩子必败）。**徽章刷新 1965 → 1991**（1893 Py + 98 FE），§0/§2 同步。⚠️ 分块清单生成须用 `NR%4==0..3`（`NR%4==1..4` 第四组恒空 → pytest 无参跑全量污染口径，本窗实踩两次）。
- **测试口径注记（2026-09-22 二十一次刷新）**：R3-W4 部署收口——**1868 收集 / 1867 通过 / 1 跳过 / 0 失败**（`a0f0af2` 迁移 4 例 + `5cc70ee` CI 用例修复；服务器分块实测 **371+1跳过 / 522 / 585 / 385** + 迁移文件单跑 4/4，`7578e57`；GitHub CI run 35714334710 全量绿；41 卡在位；三端 blob 抽验 6/6 SAME）。**徽章随本批部署刷新：1847 → 1965**（1867 Py + 98 FE），§0/§2 同步。
- **测试口径注记（2026-09-22 二十次刷新）**：系统 Python 3.12 实测 **1864 收集 / 1863 通过 / 1 跳过 / 0 失败**（v1.38 遗留收口小块：hub 角色后缀解析收口 + round3 文本断言清零转行为断言，净增 3 例；分块 **470 + 430 + 564 + 400** 精确吻合；41 卡在位、工作树仅本批文件；ruff 全仓 0 错；徽章=通过数口径 **1847**，待部署批随徽章刷新）。
- **测试口径注记（2026-09-22 十九次刷新）**：系统 Python 3.12 实测 **1861 收集 / 1860 通过 / 1 跳过 / 0 失败**（十九次刷新：六域三次排查根治批 v1.38 收口，含刻度混用 P0 读写两侧 + 阶段落盘/键空间 P1 + 假接线四处；分块 **467+1跳过 + 430 + 563 + 400** 精确吻合；41 卡在位、工作树仅本批改动；突变验红 3/3；ruff 全仓 0 错 + ci_gates 4/4；徽章=通过数口径 **1847**，待下批部署时随徽章刷新）。
- **测试口径注记（2026-09-22 十八次刷新）**：系统 Python 3.12 实测 **1750 收集 / 1749 通过 / 1 跳过 / 0 失败**（十八次刷新：六域根治四块净增用例；分块 **461 + 382 + 553+1 + 353** 精确吻合；服务器同口径 **465 + 362 + 544+1 + 378** 全绿；vitest 98/98；徽章=通过数口径 **1847**）。
- **测试口径注记（2026-09-22 十七次刷新·声明式）**：身份拷问剧本批（`86af331`）+5 用例（`tests/test_identity_interrogation_defense.py` 5/5 绿 + 突变验红 2 例命中 + 受影响面 153 通过 + ruff 0 错 + 生产探针：智谱在剧本下答「哼，行吧，那我就是十四——会惦记你、也会嫌你烦的那种」）。**全量基线未刷新**：本检出 --collect-only 1748 = 1689（十六次载明）+ 未归因增量 54（`d53611e` 收尾批自带测试与并行窗在制品合计）+ 本批 5；工作树含并行窗口在制品 16 项，按口径纪律基线重测留待并行窗收口后统一执行。
- **测试口径注记（2026-09-21 十六次刷新）**：系统 Python 3.12 实测 **1689 收集 / 1688 通过 / 1 跳过 / 0 失败**（十六次刷新：selftalk 四项修复 cherry-pick 移植（`988cf9d`）+ 知识槽身份自源过滤（`743a98e`）净增 32 用例，4 处旧 skip 随用例重写吸收；分块 **500 + 314 + 485+1 + 389** 精确吻合；服务器同步全绿 **501 + 336 + 483+1 + 368**；vitest 98/98）。（十五次刷新 **1657/1653/4**，v1.35 收仓口径统一，分块 **341 + 387+3 + 494+1 + 431**）。（十四次 **1615/1611/4**，战役终局，分块 **410 + 380+1 + 406 + 415+3**）。（十三次 **1455/1451/4**）。（上批十二次 **1448/1444/4**）。
- **测试口径注记（2026-09-20 十一次刷新）**：系统 Python 3.12 实测 **1436 收集 / 1432 通过 / 4 跳过 / 0 失败**（十一次刷新：全仓历遍批次 +7 用例 = `tests/test_async_bridge_contract.py` 5 + 信封位次 1 + 批量 GC 真执行 1；主检出分块 **314 + 384+3 + 330+1 + 404** 精确吻合）。（上批十次刷新 **1414 收集 / 1410 通过**）。（上批九次刷新 **1351 收集 / 1347 通过**）。（上批八次刷新 **1328 收集 / 1324 通过**）。（上批七次刷新 **1323 收集 / 1319 通过**）。（上批六次刷新 **1307 收集 / 1303 通过**）+ 前端 vitest **98/98**（16 文件）。
  ...
  ```
  四块结果相加应等于 `--collect-only` 的收集数（当前检出 **1689** = **500 + 314 + 485+1 + 389** 精确吻合，2026-09-21 selftalk 四项移植后分块实跑；收集数随 `config/characters/` 卡数浮动 = 2 × 卡数 + 7，**现役 41 张**。`test_knowledge_routes_index.py` 已在分块清单内）。上批十五次口径 **1657** = **341 + 387+3 + 494+1 + 431**。
...
| 版本 | 日期 | 变更 |
| **v1.38** | **2026-09-22** | **六域三次排查根治**（好感度刻度混用 P0 + EmotionStage 键空间/落盘 P1 + 键口径 P2）：点存/审计镜像/回放统一 shisi 刻度（读侧按 reason 标记区分双刻度，round-trip 用例 + 突变验红 3/3 锁死）；阶段只用 track 键 + emotion_stage_state 真落盘 + evaluate 路由改纯查询；affinity_state 空 user 键同构；conftest 隔离拆独立 try（防 ase_hub 导入失败静默跳过库重定向）。收口实测 **1861 收集 / 1860 通过 / 1 跳过 / 0 失败**（四分块 467+1 / 430 / 563 / 400 精确吻合，41 卡在位）+ ruff 全仓 0 错 + ci_gates 4/4 |
| **v1.37** | **2026-09-22** | **六域二次全面排查根治（五块分块验收）**——用户指令「昨日进行了重大更新多次，需要你继续全面排查…」→ 第二轮排查 6 P0 / 9 P1 / 10 P2 → 裁决「按推荐全面进行」。**块A/P0**（`04c7ed2`）：情绪衰减基准落盘+缺失回填（首版以 `None` 作哨兵，而 `if hours > 0` 永假且推进赋在 if 内 → 基准永停 None，衰减换种死法仍未发生）；提醒时刻校验改本地墙钟。**块B/P0**（`3ec093f`）：投递异常不再被日志通道掩盖；`main.py` 入口 websocket 改定向。**块C/P0**（`af5c41e`）：出站消息归属全链贯通（assistant 行无 character_id → 切角色继承他人台词）+ 存量空归属懒回填（旧 `OR character_id=''` 在存量全空时恒真=过滤整体失效，生产 88% 空归属）+ `run_api._character_resolver` 的 `"default"` truthy 缺陷。**块D/P1**（`c37d355`）：`dry_run` 死路径根治（闸门前置使 `generate_with_llm` 整条不可达）；决策层接地补 `last_user_message`/`response_rate`；`note_user_interaction` 就近落盘。**块E/P1+P2**（`04229eb`，14 文件 456+/67-）：⑩节流账本落盘（4 账本，逐字段容错+NaN/inf 钳除+上限 200；旧纯内存重启归零 → 退避永停第 1 档、等待窗作废、每日去重失效、祝福重发）；⑪`ase_engine.save_state` 走 `json_state.atomic_write_json`；⑫**好感度双真源根治**（`mapper.sync` 收会话键 vs `_persist_affinity` 收裸 user_id，两套键空间按构造零交集，生产实证 129 行 5 键 vs 3 键无一重合，enhancer 重启回放拿不到数据；统一 `user_key_from_session`+双写+点存兜底+`default_db_path()` 共享 owner）；⑬**`semantic_memory` 向量召回读侧协程缺陷**（`async def _search` 被同步直调 → coroutine 恒真使 `or []` 不触发 → 向量召回恒空，谎言家族新例；改 `inspect.isawaitable`+`run_async`）；⑭反思按会话检索空归属死结（下推 WHERE 使空归属历史连候选都进不来 → 改取回后 Python 过滤+丢弃计数）；⑮`get_cross_session_tail` 排序改 `id DESC`；⑯`time_awareness_tool` 默认日期改 `now_local().date()`；⑰删死参 `character_overrides`；⑱激活角色缓存键补 `id(_list_all_characters)`。**🔴 自伤事故与根治**：块E 落盘后未隔离夹具把测试键写进开发机真实 `data/scheduler_config.json`（gitignored → 不被 git 发现却反向改变用例行为）→ conftest 增 autouse 夹具 `isolate_runtime_state_files`。**验证**：分块 **1839 收集/1838 通过/1 跳过/0 失败** + ruff 0 + ci_gates 4/4；**基线对照 21 失败 → 0**；突变验红 **5/5** 命中、还原残留 0；新增 `tests/test_block_e_hardening.py` 27 例。**遗留**：生产 affinity 双键空间历史数据不迁移、审计表测试遗留键未清、本批未部署（归用户裁决）|
|------|------|------|
| **v1.36** | **2026-09-22** | **六域全面排查根治**（用户裁决：按推荐全面修复+分块验收+真删除）：①遗忘模型 days_old 恒 30（naive/aware 相减 TypeError 被吞）→`_fact_age_days` 按龄遗忘复活；②set_reminder trigger_time 三关校验（畸形串永不到期残留通道根治）；③情绪衰减基准独立记账（旧传 ≈0.08h 名存实亡）；④AffinityEnhancer 审计回放恢复（重启归零 ±3 爬坡/解锁重复根治）+setup_shisi db_path 接线；⑤重要日期按画像生日+逐用户定向（旧全局广播+dedup 无用户维度）；⑥事实检索过滤下推 SQL/Chroma；⑦pending_events 死链整拆（CrossSessionReasoner 删文件+4 接线+DROP 表）；⑧ledger prune 画像豁免+投影取最新；⑨DB 版 WorkingMemory+四死表+affinity_log CRUD+get_stats 真删；⑩web 投递按会话定向收口+提醒角色名按会话解析+query_reminders 无 meta 拒绝。测试 **1750/1749/1** 双端全绿+vitest 98+ruff 0；端点 220/186 不变；突变验红 8/8；DELETION_LOG 2026-09-22 节 |
|------|------|------|
