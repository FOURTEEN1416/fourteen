...
- **技术栈**：Python 3.10+ / React 19 / Vite 8 / TypeScript 6 / Tailwind 4 / Zustand 5 / 测试 1422（1324 Py 通过 + 4 跳过，98 FE 通过；2026-09-20 系统 Python 3.12 + vitest 实测，见 §4.3）/ MIT
- **核心能力**（见 `README.md`）：
...
| 安全 | 安全开发 | `security/` |
| 测试 | QA | `tests/`（1324 Python 测试通过 + 4 跳过；前端 98 测试；2026-09-20 实测，口径同 CODE_GRAPH §1.1) |
...
- 626+ 测试已建立，新功能必须附测试（实测基线:1324 Python 通过 + 4 跳过 / 98 前端，2026-09-20 实跑,与 CODE_GRAPH §1.1 对齐)
- 用户明确反对采样验证，要求完整验证
- **测试口径注记（2026-09-20 八次刷新）**：系统 Python 3.12 实测 **1328 收集 / 1324 通过 / 4 跳过**（八次刷新：本批 +5 = 复核补漏（`local_day_utc_bounds` 换算契约 3 + `StructuredMemory` 本地日窗口真实 SQL 边界 2）；分块实跑四块 **241+386+328+369** 与收集精确吻合）。⚠️ **口径注记（2026-09-20 复核批补充）**：本数字为**工作树快照**——测量时工作树**同时含并行窗口的在制品测试**（其另有 `frequency.py` 配额日界 UTC→本地、`analytics.py`、`important_dates.py`、`memory_pipeline.py` 系统错误占位过滤等未提交改动）；该批落地后收集数已继续变化（复核时实测 1332）。与「`config/characters` 被 gitignore 致 persona 参数化基数不可复现」属**同类**：**引用测试基线必须同时声明工作树状态**。（上批七次刷新 **1323 收集 / 1319 通过**：本批 +16 墙钟时区修复回归 —— `tests/test_local_time.py` 13【now_local 时区契约 2 / `_is_late_night` 边界参数化 6 + 语义钉死 1 / `TimeContext` 本地时钟 2 / 静态防护 1 / `_local_now` 委托 1】+ `tests/test_memory_pipeline.py` 3【喂本地时钟 1 + 深夜加权正/反 2】；分块实跑四块 **241+386+328+364** 与收集精确吻合）（上批六次刷新 **1307 收集 / 1303 通过**：+34 提醒意图管线回归 test_reminder_intent_pipeline 15 晋级线参数化 + 7 终审三分支/防假承诺 + 3 澄清状态机 + 6 到期投递 + 2 工具层 + 1 老库迁移）（上批五次刷新 +14 回复质量根治回归 test_reply_quality_overhaul）（更早 +168 = 本批角色卡扩充 +34【16 新卡 × persona 参数化 2 用例】+ 知识库修复回归 5【交错合并 2 + knowledge 路由建索引 3】+ 09-19 晚通道隔离等先前提交增量约 131 未同步文档）+ 前端 vitest **98/98**（16 文件）。**角色卡现役 41 张**（25 既有 + 16 文学导入），persona 注入参数化用例数 = 2 × 卡数，卡数变动会动基数。
  ...
  ```
  四块结果相加应等于 `--collect-only` 的收集数（当前 **1328**；2026-09-20 复测四块 241+386+328+369 精确吻合，`test_knowledge_routes_index.py` 已在分块清单内，无需再单独跑）。
