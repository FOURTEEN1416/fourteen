...
> **版本**：v1.15（2026-09-20 提示词构建行业对齐批次：全部角色卡移除 scenario 字段（防开场锚定，根因级修复）；参照 SillyTavern 默认序列与 chara-card-spec-v2 重排 prompt——扮演规则移至对话历史之后（post-history 位，权重最高）、新增对话示例段（mes_example 历史之前 few-shot）、知识库双重注入去重、orchestrator 人设片段精简为身份绑定；上一批次 v1.14 角色完善与文学导入（角色卡 25→41 张——《我的26岁女房客》《从你的全世界路过》《云边有个小卖部》《某某》《天堂旅行团》主要角色 16 张新卡全字段落库，既有 25 卡补数值字典/mes_example/修复伊蕾娜损坏字段，知识索引重建脚本补透传 + 检索双路交错合并修复，41 索引全量重建；上一批次 v1.13 主动消息「静默时段空耗配额」根因修复，v1.12 全仓扫描·文档对齐，v1.11 体验修复——人设注入三字段/主动消息连锁死锁/agnes 替换 sensenova/知识索引重建+BM25）
...
- **技术栈**：Python 3.10+ / React 19 / Vite 8 / TypeScript 6 / Tailwind 4 / Zustand 5 / 测试 1353（1255 Py 通过 + 4 跳过，98 FE 通过；2026-09-20 系统 Python 3.12 + vitest 实测，见 §4.3）/ MIT
- **核心能力**（见 `README.md`）：
...
| 安全 | 安全开发 | `security/` |
| 测试 | QA | `tests/`（1255 Python 测试通过 + 4 跳过；前端 98 测试；2026-09-20 实测，口径同 CODE_GRAPH §1.1) |
...
- 626+ 测试已建立，新功能必须附测试（实测基线:1255 Python 通过 + 4 跳过 / 98 前端，2026-09-20 实跑,与 CODE_GRAPH §1.1 对齐)
- 用户明确反对采样验证，要求完整验证
- **测试口径注记（2026-09-20 四次刷新）**：系统 Python 3.12 实测 **1259 收集 / 1255 通过 / 4 跳过**（四次刷新同日二更：本批 +5 结构回归 TestSystemPromptStructure）（较上批 +168 = 本批角色卡扩充 +34【16 新卡 × persona 参数化 2 用例】+ 知识库修复回归 5【交错合并 2 + knowledge 路由建索引 3】+ 09-19 晚通道隔离等先前提交增量约 131 未同步文档；分块实跑合计约 235s）+ 前端 vitest **98/98**（16 文件）。**角色卡现役 41 张**（25 既有 + 16 文学导入），persona 注入参数化用例数 = 2 × 卡数，卡数变动会动基数。
  ...
  ```
  四块结果相加应等于 `--collect-only` 的收集数（当前 1259，另有 test_knowledge_routes_index.py 3 例不在分块清单内需单独跑）。
