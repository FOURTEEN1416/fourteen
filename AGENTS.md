...
| 安全 | 安全开发 | `security/` |
| 测试 | QA | `tests/`（1042 Python 测试 + 75 前端测试 = 1117,2026-09-01 实测全过,口径同 CODE_GRAPH v3.5.0) |
...
- 用户明确反对采样验证，要求完整验证
- **测试口径注记（2026-09-17 刷新）**：文档口径 1117（1042 Python + 75 前端，2026-09-01 .venv 实测）——该环境随 09-14 主仓事故丢失；当前可用口径为系统 Python 3.12 实测 **1014 收集/1010 通过/4 跳过**（09-17：995 基线 + 三连修 13 用例 + 开关 3 用例 - 死代码清洗指令测试 20 用例；前端 vitest **87/87**、tsc 0 错），详见 `docs/verification/2026-09-17-三连修复验证报告.md`。跑测试：`PYTHONPATH= python -m pytest -q`
- 验证报告：归档到 `docs/`
