...
> **版本**：v1.12（2026-09-19 全仓扫描·文档对齐批次：页面口径 17、语音行去 Edge-TTS 残留；上一批次 v1.11 体验修复——人设注入三字段/主动消息连锁死锁/agnes 替换 sensenova/知识索引重建+BM25）
...
- **技术栈**：Python 3.10+ / React 19 / Vite 8 / TypeScript 6 / Tailwind 4 / Zustand 5 / 测试 1147（1060 Py 通过 + 4 跳过，87 FE 通过；2026-09-18 系统 Python 3.12 + vitest 实测，见 §4.3）/ MIT
- **核心能力**（见 `README.md`）：
...
| 安全 | 安全开发 | `security/` |
| 测试 | QA | `tests/`（1060 Python 测试通过 + 4 跳过；前端 87 测试；2026-09-18 实测，口径同 CODE_GRAPH §1.1) |
...
- 626+ 测试已建立，新功能必须附测试（实测基线:1060 Python 通过 + 4 跳过 / 87 前端，2026-09-18 实跑,与 CODE_GRAPH §1.1 对齐)
- 用户明确反对采样验证，要求完整验证
