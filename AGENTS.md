...
- **技术栈**：Python 3.10+ / React 19 / Vite 8 / TypeScript 6 / Tailwind 4 / Zustand 5 / 测试 1515（1417 Py 通过 + 4 跳过，98 FE 通过；2026-09-20 系统 Python 3.12 + vitest 实测，见 §4.3）/ MIT
- **核心能力**（见 `README.md`）：
...
| 安全 | 安全开发 | `security/` |
| 测试 | QA | `tests/`（1410 Python 测试通过 + 4 跳过；前端 98 测试；2026-09-20 实测，口径同 CODE_GRAPH §1.1) |
...
- 626+ 测试已建立，新功能必须附测试（实测基线:1410 Python 通过 + 4 跳过 / 98 前端，2026-09-20 实跑,与 CODE_GRAPH §1.1 对齐)
- 用户明确反对采样验证，要求完整验证
- **测试口径注记（2026-09-20 十次刷新）**：系统 Python 3.12 实测 **1414 收集 / 1410 通过 / 4 跳过**（十次刷新：包 Q A/B/C 收编 +63（含 `tests/test_abc_*.py`×5）；主检出分块 **396 +323 +373+3 +318+1** 精确吻合）。（上批九次刷新 **1351 收集 / 1347 通过**）。（上批八次刷新 **1328 收集 / 1324 通过**）。（上批七次刷新 **1323 收集 / 1319 通过**）。（上批六次刷新 **1307 收集 / 1303 通过**）+ 前端 vitest **98/98**（16 文件）。**角色卡现役 41 张**（25 既有 + 16 文学导入），persona 注入参数化用例数 = 2 × 卡数，卡数变动会动基数。
  ...
  ```
  四块结果相加应等于 `--collect-only` 的收集数（当前 **1414**；2026-09-20 复测四块 396 +323 +373+3 +318+1 精确吻合，`test_knowledge_routes_index.py` 已在分块清单内，无需再单独跑）。
