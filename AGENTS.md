...
- **技术栈**：Python 3.10+ / React 19 / Vite 8 / TypeScript 6 / Tailwind 4 / Zustand 5 / 测试 1401（1303 Py 通过 + 4 跳过，98 FE 通过；2026-09-20 系统 Python 3.12 + vitest 实测，见 §4.3）/ MIT
- **核心能力**（见 `README.md`）：
...
- 用户明确反对采样验证，要求完整验证
- **测试口径注记（2026-09-20 六次刷新）**：系统 Python 3.12 实测 **1307 收集 / 1303 通过 / 4 跳过**（六次刷新：本批 +34 提醒意图管线回归 test_reminder_intent_pipeline 15 晋级线参数化 + 7 终审三分支/防假承诺 + 3 澄清状态机 + 6 到期投递 + 2 工具层 + 1 老库迁移；分块实跑四块 373+336+391+203 与收集精确吻合，合计约 207s）（上批五次刷新 +14 回复质量根治回归 test_reply_quality_overhaul）（更早 +168 = 本批角色卡扩充 +34【16 新卡 × persona 参数化 2 用例】+ 知识库修复回归 5【交错合并 2 + knowledge 路由建索引 3】+ 09-19 晚通道隔离等先前提交增量约 131 未同步文档）+ 前端 vitest **98/98**（16 文件）。**角色卡现役 41 张**（25 既有 + 16 文学导入），persona 注入参数化用例数 = 2 × 卡数，卡数变动会动基数。
