...
> **版本**：v1.13（2026-09-19 主动消息「静默时段空耗配额」根因修复：记账与投递解耦、静默前置到生成层、LLM 输出清洗、去重与可观测性；上一批次 v1.12 全仓扫描·文档对齐，v1.11 体验修复——人设注入三字段/主动消息连锁死锁/agnes 替换 sensenova/知识索引重建+BM25）
...
- **技术栈**：Python 3.10+ / React 19 / Vite 8 / TypeScript 6 / Tailwind 4 / Zustand 5 / 测试 1169（1082 Py 通过 + 4 跳过，87 FE 通过；2026-09-19 系统 Python 3.12 + vitest 实测，见 §4.3）/ MIT
- **核心能力**（见 `README.md`）：
...
| 安全 | 安全开发 | `security/` |
| 测试 | QA | `tests/`（1082 Python 测试通过 + 4 跳过；前端 87 测试；2026-09-19 实测，口径同 CODE_GRAPH §1.1) |
...
- 626+ 测试已建立，新功能必须附测试（实测基线:1082 Python 通过 + 4 跳过 / 87 前端，2026-09-19 实跑,与 CODE_GRAPH §1.1 对齐)
- 用户明确反对采样验证，要求完整验证
- **测试口径注记（2026-09-19 三次刷新）**：系统 Python 3.12 实测 **1086 收集 / 1082 通过 / 4 跳过**（较上批 +22 = 主动消息配额/投递解耦批次新增 22 个回归用例，全部在 `tests/test_proactive.py`；分块实跑合计 **164.2s**）+ 前端 vitest **87/87**（15 文件）+ `tsc --noEmit` 0 错误。
