...
- **技术栈**：Python 3.10+ / React 19 / Vite 8 / TypeScript 6 / Tailwind 4 / Zustand 5 / 测试 **1539**（**1451** Py 通过 + 4 跳过，98 FE 通过；2026-09-21 P1 分块实测，收集 **1455**；角色卡 **41 张**）/ MIT
- **核心能力**（见 `README.md`）：
...
| 安全 | 安全开发 | `security/` |
| 测试 | QA | `tests/`（**1451** Python 通过 + 4 跳过；前端 **98**；2026-09-21 P1 分块实测收集 **1455**；现役角色卡 41 张） |
...
- 626+ 测试已建立，新功能必须附测试（实测基线：**1451** Python 通过 + 4 跳过 / 98 前端；2026-09-21 P1 分块实测收集 **1455**，0 失败；现役角色卡 **41 张**）
- 用户明确反对采样验证，要求完整验证
- **测试口径注记（2026-09-21 十三次刷新）**：系统 Python 3.12 实测 **1455 收集 / 1451 通过 / 4 跳过 / 0 失败**（十三次刷新：P1 ASE/affinity/情景层 +7 用例；分块 **341 + 366+4 + 325 + 419**）。（上批十二次 **1448/1444/4**）。（上批十一次 **1436/1432/4**）。
- **测试口径注记（2026-09-20 十一次刷新）**：系统 Python 3.12 实测 **1436 收集 / 1432 通过 / 4 跳过 / 0 失败**（十一次刷新：全仓历遍批次 +7 用例 = `tests/test_async_bridge_contract.py` 5 + 信封位次 1 + 批量 GC 真执行 1；主检出分块 **314 + 384+3 + 330+1 + 404** 精确吻合）。（上批十次刷新 **1414 收集 / 1410 通过**）。（上批九次刷新 **1351 收集 / 1347 通过**）。（上批八次刷新 **1328 收集 / 1324 通过**）。（上批七次刷新 **1323 收集 / 1319 通过**）。（上批六次刷新 **1307 收集 / 1303 通过**）+ 前端 vitest **98/98**（16 文件）。
  ...
  ```
  四块结果相加应等于 `--collect-only` 的收集数（当前检出 **1455** = **341 + 366+4 + 325 + 419** 精确吻合，2026-09-21 P1 分块实跑；收集数随 `config/characters/` 卡数浮动 = 2 × 卡数 + 7，**现役 41 张**。`test_knowledge_routes_index.py` 已在分块清单内）。
