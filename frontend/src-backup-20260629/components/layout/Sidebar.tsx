  ...
  return [
    {
      ...
      items: [
        ...
        { to: '/roles', icon: Users, label: '角色配置页面', badge: '新' },
      ],
    },
    {
      label: '用户/角色',
    ...
    },
    {
      ...
      items: [
        ...
        ...(isAdmin ? [{ to: '/admin/users', icon: Shield, label: '用户管理', badge: 'admin' }] : []),
        ...
        {level === 'global' && (
          <>
            {globalNavGroups.map((group) => (
              <div key={group.label}>
                {/* Drawer group header (clickable to toggle) */}
                {group.drawer ? (
                  !collapsed && (
                    <>
                      <button
                        ...
                        className="nav-item flex items-center gap-3 rounded-lg px-3 py-2 text-sm text-gray-500 hover:text-gray-700 w-full cursor-pointer"
                        data-hover="blue"
                      >
                        <User className="w-4 h-4 shrink-0" />
                        <span>{group.label}</span>
                        <ChevronDown className={`w-3 h-3 ml-auto transition-transform ${drawerOpen ? '' : '-rotate-90'}`} />
                      </button>
                      <div className={`space-y-0.5 px-1.5 mt-0.5 overflow-hidden transition-all duration-300 ${drawerOpen ? 'max-h-96 opacity-100' : 'max-h-0 opacity-0'}`}>
                        {group.items.map(({ to, icon: Icon, label, end }) => (
                          <NavLink
                            ...
                            className={({ isActive }) =>
                              `nav-item flex items-center gap-3 rounded-lg px-3 py-2 text-sm transition-all duration-200 ${
                                isActive
                        ...
                        >
                          ...
                          {!collapsed && (
                            <>
                              <span>{label}</span>
                              {badge && (
                                <span className={`ml-auto text-[9px] rounded px-1 ${badge === '新' ? 'tag-pink' : 'tag-pink'}`}>
                                  {badge}
                                </span>
                              )}
                            </>
