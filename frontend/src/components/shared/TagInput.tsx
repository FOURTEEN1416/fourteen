...
export default function TagInput({ tags, placeholder = '输入后回车添加', onChange }: TagInputProps) {
  ...
  return (
    <div className="flex flex-wrap items-center gap-1.5 px-3 py-1.5 glass-card rounded-lg min-h-[36px]">
      {tags.map((tag, i) => (
        <span key={i} className="inline-flex items-center gap-1 bg-primary-100/50 text-primary-700 text-xs rounded-md px-2 py-0.5">
          {tag}
          <button onClick={() => remove(i)} className="hover:text-primary-500 transition-colors">
            <X className="w-3 h-3" />
          </button>
        </span>
      ))}
