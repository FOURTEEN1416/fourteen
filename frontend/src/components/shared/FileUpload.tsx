  ...
  return (
    <div>
      ...
      >
        ...
        {accept && <p className="text-[10px] text-gray-400 mt-1">支持 {accept} 格式</p>}
        {maxSize && <p className="text-[10px] text-gray-400">单文件 ≤ {maxSize}MB</p>}
        <input
          ref={inputRef}
          type="file"
          accept={accept}
        ...
        <div className="mt-2 space-y-1">
          {files.map((f, i) => (
            <div key={`${f.name}-${f.size}-${f.lastModified}-${i}`} className="flex items-center gap-2 glass-card rounded-lg px-3 py-1.5 text-xs">
              ...
              <button onClick={() => remove(i)} className="text-gray-400 hover:text-red-400 transition-colors">
                <X className="w-3 h-3" />
              </button>
            </div>
          ))}
