...
export default function MonitorPage() {
  ...
  const activeId = selected || (characterList.find((c: any) => c.is_active)?.character_id ?? characterList[0]?.character_id ?? '')
      ...
      <div className="flex items-center justify-between">
        ...
        {characterList.length > 0 && (
          <div className="relative">
            ...
            >
              {characterList.map((c: any) => <option key={c.character_id} value={c.character_id}>{c.name}</option>)}
            </select>
        ...
        <>
            ...
            <Card>
              ...
              {stage ? (
                <div className="space-y-3">
                  ...
                  <div className="text-xs text-gray-500">
                    可用功能: {stage.features.map((f: string) => <Badge key={f} className="mr-1">{f}</Badge>)}
                  </div>
                </div>
              ) : <p className="text-xs text-gray-400">暂无数据</p>}
            </Card>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <Card>
              ...
              {!unlocksData?.unlocks || unlocksData.unlocks.length === 0 ? (
                <EmptyState icon="🔓" title="暂无解锁事件" description="好感度提升后解锁的功能将在此显示" />
              ) : (
                <div className="space-y-2">
                  {unlocksData.unlocks.map((u, i) => (
