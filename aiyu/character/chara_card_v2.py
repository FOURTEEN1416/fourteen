"""chara_card_v2解析器 — 支持标准V2/V3 + 爱语APP prompts格式 + SillyTavern格式。"""
...
logger = logging.getLogger("aiyu.character.chara_card_v2")
...
class AiyuPromptsParser:
    """爱语APP prompts格式解析器 — 将爱语导出的prompts结构转为标准CharaCardV2"""

    @staticmethod
    def parse(data: dict[str, Any]) -> CharaCardV2:
        prompts = data.get("data", {}).get("prompts", {})
        if not prompts:
            raise ValueError("爱语prompts格式中无prompts数据")
...
class ParserDispatcher:
    ...
    _parsers: list[tuple[type, str]] = [
        (AiyuPromptsParser, "aiyu_prompts"),
