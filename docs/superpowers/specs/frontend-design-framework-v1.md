...
> **📋 文档状态卡**（2026-08-26 治理标注；2026-09-19 刷指针 · 决策链见 `docs/DECISION_LEDGER.md`）
> - **层级**：L2 决策档案 | **结局**：🔄 部分生效
> - **✅ 生效部分**：页面骨架与信息层级、页面职责划分（时点 15 页，现行 17 页：`/intro` 产品介绍页与 `/psych` 心理画像页后增）
> - **❌ 失效部分**：三级侧边栏章节随 ADR-0011 废止（用户 08-24 否决）；StatusCenter 富化规划 → **SP-1 已于 09-01 收官**（情绪洞察卡+成就卡落地）；TTS 引擎选择器（edge-tts/gpt-sovits/bert-vits2）随 07-27 reinit + 08-28 MiMo-only 两轮收敛废止，前端仅 MiMo
> - ~~现行功能实况以 `docs/FEATURE_MAP.md` 为准~~ → FEATURE_MAP 已删（08-28 用户裁决），现行以 `docs/FUNCTION_INVENTORY.md` + `CODE_GRAPH.md` v3.8.2 为准，正文保持原样供追溯。
