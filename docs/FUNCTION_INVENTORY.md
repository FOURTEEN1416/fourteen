...
| WECHAT-3 | 断开态展示（⚠️ SP-5 在册：空态裸奔，`运行 —`/裸 `0`，无插画与行动指引） |
...
| 编号 | 功能点 |
|------|--------|
| ROLES-1 | 角色卡网格：名称/描述/前 3 锚点标签（`i%3` 轮转配色 ⚠️ SP-5 在册）/激活态徽章 |
| ROLES-2 | 设为活跃（activateCharacter + queryClient 失效）；活跃卡无按钮仅徽章 |
...
| # | 差距 | 历史出处 | 处置建议 |
|---|------|---------|---------|
| GAP-1 | 企业微信通道未实现（个人微信/控制台/WS 已实现） | 05-19 §1.2 多通道接入 | 需用户裁决是否立项（工具侧 wxwork 解密能力在 wechat-decrypt） |
