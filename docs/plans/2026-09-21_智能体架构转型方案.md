...
| 极薄安全闸（非性格硬编码） | 仅系统级：① LLM=`false` 则不发；② **投递成功才 commit**（配额/冷却）；③ 可选日上限**只为控成本**（配置默认可调/可关）——**不是**「初识必须 48h」这类人格表 | 防接口滥用与静默扣配额重演 | 把安全闸写成性格参数：违背本裁决 |
...
| D5 | **主动消息 LLM 决定时机与内容，禁止硬编码** | §B3 主动行；P1 契约 |
...
系统：false→不发；true→投递成功才 commit；超长/泄漏清洗仍要
账本：proactive_send | proactive_skip + reason
```

**明确不用**：关系阶段×小时数表、urgency 数值直接当是否发送的唯一闸（urgency 仅可作 prompt 上下文）。
