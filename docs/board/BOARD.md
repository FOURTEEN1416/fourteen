...
- **⚑ B1-B6 六条新发现真实缺陷**（此前均无记录，按真实暴露面分级）：**B3（中）`config/shisi.yaml` 的 `memory:` 整段 5 键零消费点**（`get_config("memory",…)` 全仓 0 命中；`working_memory_capacity: 20` 与硬编码 `20` 数值巧合，**掩盖未接线**；`similarity_threshold: 0.85` 正是 D3 要用的门槛，属"差一步"）；**B5（中）遗忘是物理删除**（`delete_fact`，无保底留痕）；**B4（中）`access_count` 只读不自增、指数模式不参与**→ 回忆强化不存在；**B1（中·潜伏）三套 time-of-day 分类器 + `TimeContext.now()` 用 UTC**（生产 `prompt_mode: layered` 故**未生效**，但切 `enhanced` 即全量错位；`app.timezone: Asia/Shanghai` 零读取）；**B6（低-中）四套亲密度刻度**（0-8 / 0-500 / 0-100 / 解锁 25-50-75-90，各自自洽故**只登记不重构**）；**B2（低）`build_time_context` 死代码**。
- **待用户裁决 J1-J6**：P0 是否前置 B1/D5+B3 接线；遗忘是否改"降级留痕"（数据只增不减）；心光容量 5/阈值 0.58 是否上生产；是否撤掉"用户伤心→提高打扰意愿"规则；死代码是否清理；`plasticity` 三档是否采纳。
