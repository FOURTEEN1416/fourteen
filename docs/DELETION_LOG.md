...
- 删除人称猜测式输出截断、摘要按条数缓存、历史空角色懒认领、反思共享结果缓存、向量先LIMIT后过滤的旁路。
- 删除伪流式先调用普通入口写全文再发送的分叉；统一定稿、传输确认及中断落库。
- 删除提醒角色名与ID双resolver、微信重复拆条发送循环、画像inflight直接丢后续更正、用户模型失败借平台凭证的分支。
- 删除事实bigram/子串自动合并、相似即丢弃新事实、部分文本删除规则；删除用ID或精确事实。夜间curator合并也统一此判据，停止每夜无依据提高全部事实置信度。
- 删除主动dry_run先生成再丢弃路径及错误说明；紧迫度早于生成更新，干跑不调用LLM。
- 存量私人历史、回收站、ADR及既有日志未清洗；本地修改未部署。
