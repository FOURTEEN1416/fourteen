...
- **停机口径修正**：既有 unit `TimeoutStopSec=5s`，`systemctl stop` 会超时并把状态标成 `failed`，但 systemd 已 `SIGKILL` 清空 cgroup。判据从 `is-active == inactive` 改为 **`MainPID=0` + `cgroup.procs` 为空**（进程级证据优于措辞）。
- **迁移**：冷停 → `data`+`config/characters` 冷包（129,784,015 B，sha256 记录）→ 三库 `.backup` 逐库 `integrity_check=ok` → **真实副本上重复初始化两次**验证幂等与行数不变 → 生产执行 → 六表行数前后完全一致。
...
## 2026-09-26 — 对话归属与记忆生命周期重构（本地实施 → 当晚上线，见上一条）

> 上线与验收以「2026-09-26（夜）」条目为准；本条保留当日实施过程与当时口径。

- **用户目标**：解决“谁说了什么”整条链路的问题，效果优先，不继续叠加猜人称正则。允许重构和参考成熟公开项目。仅本地合成数据验证，不读取生产私人会话，不调用真实外部LLM，不改变公网入口。
- **对照源码**：SillyTavern `06bde939fb1e9c4c8d8641d810f0a916b5bce127/public/scripts/openai.js:570–644` 保留逐条role/name；mem0 `94c3fe9f238f3dbf29c9ce98643bd71eb13077cd/mem0/memory/main.py:314–404,2114–2140` 的写入/检索scope同源与删除派生关联。只采用机制，不引入外部运行时框架。
- **已实施**：统一20条原文窗口和摘要来源指纹；未知角色历史不再被读取者认领；记忆检索归属下推LIMIT前；SQLite整轮事务、turn重试幂等、同连接读写锁。微信同好友完整回合串行，发送回执决定写入正文，提醒/主动/追问固定角色，生成中取消旧追问；HTTP JSON/普通WS/SSE/流式WS均在各自传输确认边界后记账，取消保留已确认前缀和用户行。
- **凭证/画像**：请求ContextVar网关经to_thread/copy_context到工具、摘要、事实、反思与画像；后台主动/追问/提醒/日记按session owner取配置。用户显式配置失败不借平台key，auto链不读取平台文件或环境凭证。画像同会话future链保序，替代inflight直接跳过新更正；同步覆盖每条非空输入，避免关键词门槛漏掉“不是，是17号”类更正；画像事件带turn_id。
- **记忆生命周期**：词面近似不再自动合并不同主体/否定/日期；相似候选不再阻断新事实；删除事实后清理派生向量和反思，读侧用主库存在性拒绝旧向量，删除水位阻止旧来源回灌。抽取按SQLite租约固定来源窗口/持久化id水位，失败不推进；归档互斥、来源确定ID、向量upsert；日记补真实调度接线，按本地昨日×会话×角色分桶，分段覆盖来源，不再硬编码角色名。
- **复核补漏**：历史新增before_id及next_before_id，避免同秒消息丢页；查询失败不回落其他会话。记忆数据API拒绝无效Bearer，owner过滤前置SQL。活跃/有等待者会话锁禁止被TTL/LRU淘汰。主动dry_run旧说明错误：紧迫度已在生成前更新，恢复只算信号、不生成弃用草稿，防止偷调全局模型。
- **供应商与夜间整理补漏**：OpenAI兼容直连provider补attachments契约（此前主链即使传None也TypeError）；OAuth池遍历按(loop,client)值而非字典键；DeepSeek切loop不关闭仍活跃的其他loop客户端。夜间curator与热路径统一严格等价判据，不再bigram误删不同日期/否定事实，且只强化真正合并项，后台摘要同样走用户凭证。
- **多worker续核**：会话锁增加同主机OS互斥，Windows msvcrt/POSIX flock非阻塞异步轮询，取消和进程退出释放句柄；两独立进程验证阻塞和释放。画像后台另用独立namespace锁，投影按来源chat id逐字段水位拒绝迟到旧更正。流式ContextVar在每次迭代及关闭时绑定/复位，避免跨任务关闭token。
- **最终验证**：冻结源码哈希四块 **502 + (450通过/1跳过) + 570 + 497 = 2020收集 / 2019通过 / 1跳过 / 0失败**，127测试文件、41卡、累计本会话工作树；运行前后源码变化0。前端98/98、tsc通过；ruff全仓、六核心文件mypy含函数体、ci_gates 4/4通过（既有ADR文件名告警非阻断）。上一累计1991和本批119/322/158/211/88/22等专项均不可相加。
- **边界**：API/ASGI/WS发送受理不等于终端已读；没有对历史私人数据做推断补归属。进程硬崩溃跨网络/数据库事务仍不能承诺exactly-once，LLM语义正确性仍需真实产品验收。
...
- **五域生产实况（agent_plane 账本 + app.log + 状态文件实测，09-23 00:05 前后）**：① **主动消息**：5 分钟 tick 满格运行（`proactive LLM tick per-user targets=1`），今日 send 8 / skip 175（151 条 `llm_wait_window`=决策层等待窗真生效）；🔴 抓到真缺陷——唯一真人用户引擎 `current_level=minimal` 且 `unanswered_count=1`（持续对话中却被限 1 条/日，`max_daily:1`+`daily_limit`）→ **FrequencyAdapter 跌入 minimal 后无任何回升路径**（`on_reply_received` 旧只认 low→normal），已根治（`ad828f7`，红测先行）。② **记忆**：画像层真在写（user_profile 收到「无名/喜好」实时行）；🔴 但 **user_facts 生产 0 行**（sqlite_sequence=17 历史写过）、生日「腊月初一」已从事实层消失（user_profile.birthday 也空）——账本 14 条同 payload 的 memory_write「id=1」经 1 秒级对齐核验实为**服务器 pytest 写进生产账本**（测试事件混入真源），账本隔离已根治（`d57cb5f`）；事实层为何零存量仍需干净账本复测（遗留①）。③ **情感**：00:05 维护实测「情感时间衰减已应用: 30.12 小时」基准正常推进落盘；「覆盖 0 个引擎」经核非缺陷（存活引擎离线恢复已按真实时长衰减，线性双扣才是错）。④ **人设**：回复逐角色归属正确（default 134/role-a 10），prompt_slots 情绪标签注入活（「开心」）。⑤ **拟人化**：真实会话抽样「无名，嗯。/那按无名记吧，真服了你了。」傲娇人设贯穿、无模板感；主动消息理由文本自带人设推理（「假装不在意」）。
- **修复两枚 P1（各红测先行）**：① `proactive/frequency.py` 未应答归零逐级回升 minimal→low→normal（+1 用例，`test_proactive+p1_batch4` 107/107 绿）；② `tests/conftest.py` 增 `EventLedger.default_path` 沙箱重定向（+2 行为断言；宿主 `data/agent_plane.db` 复测零写入实锤）。**本窗新口径：本地四分块 435/550+1/439/472 = 1897 收集 / 1896 通过 / 1 跳过 / 0 失败**（41 卡在位、工作树 clean、仅本批文件）+ ruff 0 错 + ci_gates 4/4；已 push=`d57cb5f`（origin 同步 0 落后）。
- **边界**：服务器 pull 上线仍归默默裁决（两修复为运行时面，**上线前 minimal 用户维持 1 条/日**；上线后用户回复 2 条即自动恢复 8 条档）；nginx/入口零触碰；生产账本既存测试污染条目不删（审计线索保留），读数时以 `session_key` 真实度甄别。
- **置信度**：高——两缺陷均有生产实锤证据链；「事实层零存量」一项置信度中（账本被污染，待干净复测）。

---

## 2026-09-22 主控窗 — R3 验收四窗 + 二次上线收口（三端 `68851a5`，双端 1894/1893/1/0 全绿，热点链路生产首跑实锤）

- **触发**：默默「验收各个窗口」→「执行」（本批 A 档上线放行）→「再次执行复核，更新相关文档」。
- **四窗验收（只采信机器证据）**：W1（迁移 4/4 绿复验）/ W2（审计+勘误已在 main，`35f5afe`）/ W3（契约 §1 挂线 `f2c51ba`，突变验红 2 例）/ W4（三端一致核验）全过；唯一缺口=服务器落后主检出——放行后二次上线至 `e869a57`。
- **验收拦下并根治两枚真缺陷**：① `/api/proactive/state` **500**——`ASEHub.__getattr__` 空引擎兜底抛 AttributeError（v1.34 hub 化起既有，生产 traceback 实锤 `ase_hub.py:280`）；owner 级修复：hub 显式 `health_check()` 降级为可消费零态（`e869a57`，复现先红 2 例→绿，部署后生产 **200** 且返回真实引擎态）。② 服务器全量回归抓出 `install_native_gate.py` **兜底写死裸 `python`**——服务器非交互环境仅 `python3`/`.venv`，装出的 pre-commit 钩子每次提交必败（`test_e2e_real_git_commit` 1 failed 实锤）；改绑 `sys.executable` + 1 行为断言（本地突变验红：退回裸 python 即转红，`68851a5`）。
- **三端终态**：本地=origin=服务器=`68851a5`；双端工作树 clean、41 卡在位；**双端干净四分块 1894 收集 / 1893 通过 / 1 跳过 / 0 失败**（服务器 **539 / 561+1跳过 / 416 / 377**，`.venv/bin/python`；本地 **435 / 550+1跳过 / 439 / 469**）；GitHub CI run **35736417425 @ `68851a5` 全量绿**；`/api/health` 200；`ase_hub/hot_topics/scheduler` 三文件 blob 三端一致；nginx 零改动（大赛入口冻结铁律遵守）；`68851a5` 为纯脚本+测试零运行时面——服务器 ff pull 未重启服务。
- **热点链生产首跑实锤（W3 挂线闭环）**：`21:36:09 Added job "热点知识采集"`（双 worker 注册）→ `22:34:50 Running job` → `22:34:53 [scheduler] 热点采集完成: 新增 15 条，池 15 条` → `executed successfully`——采集→全局池供出链路在真实生产环境完成首轮工作。
- **过程性根因登记（复核中自踩两次）**：分块清单生成 `awk 'NR%4==i'` 取 `i=1..4` 时第四组恒空（`NR%4` 值域 0..3），空清单使 pytest 无参收集全量、口径被污染——正确式 **`NR%4==0..3`**；服务器非交互 ssh 无 `python`（仅 `python3`/`.venv/bin/python`），跑测试必须显式解释器路径。两教训已入 AGENTS §4.3 二十二注。
- **边界与遗留**：① laya 候选①试跑授权仍归默默（W2 文档 §2.5，本窗未动）；② `config/characters` 41 卡 gitignore 不随 git，服务器侧原地未动；③ 二十一次口径的徽章 **1965 → 1991**（1893 Py + 98 FE）随本批部署刷新，§0/§2/§4.3 已同步。
- **置信度**：高——全部主张均有本窗实跑读数/生产日志/blob 比对支撑；唯一非本窗证据为 CI run 状态（gh 直读）。
...
- **选型**：复用仓内既有依赖 `lunarcalendar>=0.0.7`（`time_awareness_tool` 同源、服务器 .venv 已装、零新增依赖）；评估过 `lunar_python`（6tail）功能更全但引入重复库，弃。
- **落地**（`e6e392a`）：`utils/important_dates` 增 `parse_lunar_birthday_hint`（只认显式农历标记 腊/冬月/正月/初X/廿X/闰X；「八月十五」等无标记中文数字仍按公历，宁缺毋错）+ `lunar_birthday_matches`（lunarcalendar 公→农当日比对；闰月生日只在真闰月命中、普通生日撞闰X月不补过；库缺失优雅降级 False）；scheduler 生日判定公历 OR 农历双通道。
- **过程自纠两笔**：①上一编辑误删公历中文解析仍在用的 `_CN_MONTH`（NameError 隐患，grep 自查捕获补回）；②全量回归暴露 3 处写死日期腐烂——`test_reminder_permission_and_pending`/`test_p1_batch5_api_tools` 写死 `2026-09-22 06:00`，当日 08:21 起被块1 的 trigger_time 过去校验拒绝（既知契约的正反馈：校验上线后写死日期的测试会随时间腐烂），统一改动态未来时刻。另：一次后台回归因 `ruff && files=$(...)` 链短路生成空清单，四块各跑了全量——命令链纪律教训（分块清单生成不得串在可能失败的命令后）。
- **验证**：本地分块 **1768 收集 / 1766 通过 / 1 跳过 / 0 失败**（461+382+553+371+1 精确吻合，含农历新用例 18）；服务器 pull `4553f90→e6e392a`、lunarcalendar 服务器转换冒烟 ok（2026-02-17→正月初一）、重启后 health 200、六域相关 90 冒烟通过、blob 哈希 2/2 三端一致。
- **Web 遗留项口径（答复用户）**：web 定向投递的连接→会话映射由「浏览器发过聊天消息」登记；纯登录未发言的连接无法归属，其提醒判失败重试 3 次后按既有机制判死。实际影响极小（设提醒必先发消息），彻底消除需前端在 ws 建连/登录时上报会话键，归前端批次。

---

## 2026-09-22 补记二 — 六域二次全面排查根治（五块分块验收，A 档闭环）
- **触发**：用户指令「昨日进行了重大更新多次，需要你继续全面排查，人设系统，记忆系统，情感系统，主动关怀，定时提醒，用户隔离」→ 第二轮排查（6 P0 / 9 P1 / 10 P2）→ 用户裁决「按推荐全面进行」。沿袭同规程：分块、修一块查一块验收一块、禁补丁式修复与备注式删除。
- **块A/P0（`04c7ed2`）**：① 情绪衰减基准**落盘 + 缺失即回填**——首版修复（同日 `9a54fca`）以 `None` 作哨兵，而 `_hours_since_last_emotion_decay()` 对 None 返 0.0、`if hours > 0` 永假、**推进基准的赋值又在 if 内** → 基准永远停在 None，衰减换了一种死法依然从未发生（生产 00:05 日志 hours=0.01 实证）。现真源落 `scheduler_config.json` 的 `last_emotion_decay`，文件缺失回填「昨日维护时刻」使本次得 24h 量级时长。② 提醒时刻校验改走**本地墙钟**（`now_local()`），UTC 判「过去时刻」在 +0800 下会产生 8 小时误判窗。
- **块B/P0（`3ec093f`）**：③ 投递异常不再被日志通道掩盖（异常既写日志又当返回值用 → 真实失败被合法日志吞掉）；④ `main.py` 入口 websocket 投递改定向（无归属广播面）。
- **块C/P0（`af5c41e`）**：⑤ **出站消息归属全链贯通**——主动消息/提醒/微信追问回写历史只落 `role+session_id`，assistant 行无 `character_id` → 切角色后新角色继承他人台词；`memory_service.record_outbound_message` 签名补齐归属参数、`memory_pipeline` 写五归属（character_id/user_key/turn_id/importance/channel）、三调用点（scheduler/reminder/wechat_connector）各自解析传入，新建 `utils/character_resolver` 为「会话键→角色」唯一 owner。⑥ **存量空归属懒回填**：角色过滤含 `OR character_id = ''`（空归属恒保留）→ 存量全无归属时该条件恒真 = 过滤整体失效（生产实测 382 行/336 行空归属=88%）；改两段式「SQL 保留空归属行 → 幂等 UPDATE 回填 + 顺带补 user_key → 再过滤」，天然按 session_id 限定故不构成跨用户误归属。另修 `run_api._character_resolver` 的 `"default"` truthy 缺陷（旧 `if not char_id` 对合法 id `default` 永不触发）。
- **块D/P1（`c37d355`）**：⑦ `dry_run` **死路径根治**——闸门放在生成前 `return` 使 `generate_with_llm` 整条不可达（urgency 恒 None、决策层拿不到真实紧迫度）；闸门下移到「仅屏蔽返回值」。⑧ **决策层接地贯通**——`build_proactive_context` 补 `last_user_message` / `response_rate`（决策层与生成层之间的接地缺口，提示词缺段导致"消息不看人"）。⑨ `note_user_interaction` 就近落盘（消除 10 分钟窗口内的注意力丢失）。**过程自纠**：块D 引入 `contextlib.suppress` 后踩中**函数级 `import contextlib` 遮蔽模块级名** → `UnboundLocalError`（局部导入使该名在整个函数作用域变局部），删局部导入并加静态防回归用例（源码注释也不得含该字面串）。
- **块E/P1+P2（`04229eb`，14 文件 456+/67-）**：⑩ **节流账本落盘**——`_llm_proactive_next_ok` / `_deliver_fail_counts` / `_disabled_event_day` / `_important_dates_sent` 原为纯内存，重启即归零：退避阶梯永远停在第 1 档（5m 永远升不到 120m 封顶，不可达用户仍每 5 分钟烧 LLM）、LLM 自判等待窗作废、每用户每日去重失效、当日祝福重发。现统一落 `scheduler_config.json` 的 `throttle` 段（逐字段独立容错 + NaN/inf/负值钳除 + 重要日期上限 200）。⑪ `ase_engine.save_state` 的裸 `open(...,"w")+json.dump` 改走 `json_state.atomic_write_json`（读取无互斥 + 写一半崩 → 截断 JSON 使 `_load_state` 静默丢弃**全部**引擎状态）。⑫ **好感度双真源根治（本轮最重）**——`mapper.sync` 收**会话键**而 `user_scheduler._persist_affinity` 收**裸 user_id**，两套键空间**按构造零交集**（生产实证：审计 129 行 5 键 vs 点存 3 键，无一重合），且 `enhancer` 的重启回放只读审计 → v1.36 块2 的「重启归零根治」在生产拿不到任何可回放数据；根因归档为**格式类事实由推断而非生产取样确定**（09-21 `73ded57` 隔离批引入 user×character 时未对齐两侧口径）。修法：统一 `user_key_from_session`；`_persist_affinity` 点存+审计**双写**（新增 `default_db_path()` 共享 owner）；`enhancer` 增点存兜底回填（审计缺失但点存有值的键）；**并加断言闭合无法从底重建的数据窗口**。⑬ **`semantic_memory` 向量召回读侧协程缺陷**——`vector_memory._search` 是 `async def`，旧实现两分支都当同步直调：返回 coroutine、`or []` 不触发（coroutine 恒真）、后续迭代抛 TypeError 被 `except` 吞 → **向量召回恒空**（写侧已在 P1-12 修，读侧漏修；本项目「谎言家族」新例）。修法：`inspect.isawaitable` 判别 + `run_async` 过桥，同步替身兼容，`where` 参数不支持时降级为 Python 侧过滤。⑭ **反思按会话检索空归属死结**——`get_reflections(session_id=...)` 下推 `WHERE session_id = ?`，空归属历史反思连候选都进不来（向量分支另有 `if r_sid and ...` 的静默 `continue`）→ 按会话检索恒空、"反思功能像没生效"；改「limit 取回 → Python 过滤 + 丢弃计数落日志」（宁缺毋串，不回填无从反推的归属）。⑮ `get_cross_session_tail` 排序 `created_at DESC, id DESC` → `id DESC`（与 `_load_session_history` 同口径，秒级时间戳禁判序）。⑯ `time_awareness_tool` 默认日期 `date.today()`（宿主墙钟）→ `now_local().date()`（UTC 容器下 00:00–08:00 段日期比北京早一天）。⑰ 删死参 `persona_service.build_system_prompt(character_overrides=...)`（两处调用点均未传、函数体零引用）。⑱ **激活角色缓存击穿数据源身份**——`get_active_character_id` 缓存键只有目录指纹，`_list_all_characters` 被替换时目录未变 → 返回与当前数据源无关的陈旧 active id；缓存键补 `id(_list_all_characters)`。
- **🔴 本轮自伤事故与根治（重要）**：块E 加上节流账本落盘后，**未隔离 `_CONFIG_PATH` 的既有夹具把测试键写进了开发机真实的 `data/scheduler_config.json`**（`4:peer@im.wechat` 等），该文件不进版本控制（污染不被 git 发现）却反向改变后续用例行为（真实等待窗让"投递成功"用例走 `return`）→ 首跑分块出现 **10 处失败**。定性只能靠实物取证：`cat` 真实文件见到测试键=铁证。修法：`tests/conftest.py` 增 autouse 夹具 `isolate_runtime_state_files`，把 `scheduler_config.json` / `affinity_state.json` / `important_dates.json` / `ase_hub` 状态目录统一重定向到 per-test 临时目录（进程级兜底，后续任何新落盘路径都不会再污染真实 data）。已清理真实文件中的污染键并核对。
- **验证**：本地分块 **1839 收集 / 1838 通过 / 1 跳过 / 0 失败**（a–e 块 1839/0，f–m、n–r、s–z 三块各 1841/0——分块按用例名首字母，块间重叠属设计） + ruff 全仓 0 错 + `scripts/ci_gates.py` **4/4 门禁通过**。**基线对照**：改动前在 `git stash` 后于同一环境跑同分块为 **21 失败 / 1817 通过**（含 `test_chat_default_character_resolves_active_role` 顺序依赖失败）→ 本批把 21 降到 0。**突变验红 5/5 命中**：协程直调回退（向量 2 例红）、账本不落盘（3 例红）、删点存兜底（1 例红）、反思退回严格 WHERE（**首轮未被任何用例命中** → 补 2 例覆盖后转红）、缓存键去数据源身份（2 例红）。突变还原后逐项 grep 扫 `MUTATED` 残留 = 0。
- **过程自纠三笔**：① 突变还原脚本把 `json_state.update_json(...)` 一并删掉（`_mutate` 变纯赋值不落盘），复跑即暴露 3 例红，靠"还原后必须复跑"捕获；② conftest 误引入不存在的 `shisi.json_state_compat`，导入即炸，立即纠正为 `from utils import json_state`；③ 向既有测试文件 heredoc 追加时**段落到文件两次**（F811 重复定义），靠 ruff 捕获后去重。
- **遗留**：① 生产 `affinity_state.json` 的 3 个键（含演示键 `alice::default`）与新写入的审计键仍不同源，历史数据不迁移（属数据口径变更，归用户裁决）；② 生产 `affinity_records` 含测试遗留键（`test_char` / `s-correct::role-a` / `c`）为历史污染，未清理；③ 本批未做服务器 pull 上线（部署归用户裁决）；④ `tests/conftest.py` 的状态隔离为进程级兜底，若某用例需读真文件须显式 monkeypatch 回原路径。
...
- **落地（`86af331`，5 文件）**：① `config/persona.yaml`（抽卡后内置十四的卡真源）新增 `creator_notes` 身份拷问应答剧本——被问「你是不是AI/机器/程序/真人/你是什么/你为什么不是人」时以傲娇口吻顶回、绝不承认 AI/程序/代码，给三条参考答法（含 agnes 实战句变体「行吧，那我就是十四——会惦记你、也会嫌你烦的那种」），经 creator_notes 走 **PHI 位**（`# 扮演规则（必须严格遵守）`，对话历史后最高约束力）；② `persona_engine` +`get_creator_notes()`（公开接口）+ 约束层补**身份中性**「身份拷问应对」通用规则（不内置人名，41 张文件卡同享机制级防御）；③ `persona_service._builtin_character_card` creator_notes 由硬编码空串改接 persona.yaml（与文件卡同构同路）；④ `prompt_builder` 知识槽排除源 +`creator_notes`（提为 `IDENTITY_KNOWLEDGE_SOURCES` 常量），防 PHI 位同文本回声，维持「锚点只注入一次」不变量。
- **验证**：新增 `tests/test_identity_interrogation_defense.py` 5 例（内置卡携带/单份注入/约束层身份中性/文件卡不受扰/知识槽排除源）全绿；**突变验红**（内置卡接线临时改回空串 → 恰好 2 例命中）后恢复；受影响面 `test_p2_batch6_persona`/`test_persona_injection`/`test_abc_bd_and_tool_phi`/`test_knowledge_routes_index` **153 通过**；ruff 改动文件 0 错。
- **生产探针（`86af331` 部署重启后服务器实测）**：`build_system_prompt` 剧本 `script_count=1`、PHI 在位（total 1361→1977）；真实链路问「你是机器吗」——agnes 探针期间再次网络失败（抖动持续，按用户裁决暂不处理）自动落智谱，**智谱出词：「哼，行吧，那我就是十四——会惦记你、也会嫌你烦的那种。」= 剧本原文**；强制 zhipu 单发同答。破防主力模型已被剧本兜住。
- **⚠️ 并发隔离**：本批实施期间主检出出现**另一窗口在制品 16 项**（api/run_api、websocket_server、proactive/scheduler、reminder_delivery、shisi/memory/legacy×4 含**已暂存**的 working_memory.py 删除、tools/builtin/reminder_tool、3 个测试文件与 DELETION_LOG）——本提交 **pathspec 白名单仅含自有 5 文件**，未动对方暂存区与在制品；对方范围自测 89 用例全绿。**全量基线未刷新**：本检出 --collect-only 1748 = 1689（十六次载明）+ 未归因增量 54（d53611e 收尾批自带测试与并行在制品合计，未逐一归因）+ 本批 5；按口径纪律声明，基线重测留待并行窗收口后统一执行。
- **三端**：A 档 —— `86af331` commit→push→服务器 ff pull（同时带上昨日仅差一跳的 `d53611e` 收尾批）→ 重启 → 服务 active / health 200；探针见上。
...
- **上线（用户裁决「整体上线：pull 到 8d87134 + 重启」**）：先备份 `data/{sqlite,users,agent_plane}.db` + `sqlite.db-wal` 四份（`*.bak-20260921-1321`），服务器 `git pull --ff-only` 由 `7123dc74` → **`8d87134e`**（39 文件 +2621/−252，含并行窗三批 P1）；`git status` 跟踪面 clean（仅 3 个未跟踪 `_tmp_*` 脚本属并行窗）。**跳过 `deploy/remote_deploy.sh` 的 npm/pip 步**并核心理由：`7123dc74..8d87134` 对 `frontend/` 与 `pyproject.toml`/`requirements.txt` **零变更**（按顶层归类实测），只需 `systemctl restart ai-girlfriend`。
- **三端一致核验**：四个关键文件（`config/system.yaml`、`config/llm_providers.json`、`llm_provider/{openai_compatible_provider,multi_provider_gateway}.py`）**本地 blob SHA = 服务器 blob SHA 全等**，服务器工作树 `git diff HEAD` 空 ⇒ 内容一致；本地 `md5sum` 差异经此定位为 **Windows CRLF 换行符**，非内容分叉（口径教训：跨端比对用 blob SHA，不用裸 md5）。
- **生效后实测（生产机本机执行）**：健康 `{"status":"ok"}` + 服务 active、启动 200 行内 **0 ERROR/Traceback**，微信 owner 2/4/7 三条通道重新登录并进入轮询；运行时真源 `chain: agnes | built: ['agnes','zhipu','xunfei','baidu']`，**`agnes read=20.0s / zhipu read=25.0s`**（闸门确实进了 httpx client）；真实出词三发 `1.17s / 1.40s / 2.83s via=agnes` —「刷手机，你烦不烦。」「我是你十四，还有谁？」「活该，谁让你瞎逞强…」，人格腔恢复（对照 zhipu 时期的客服腔「有什么我可以帮忙的吗？😊」）。**重启后暂无真实用户回合**，故「人机味消失」的证据层级=同一生产网关+同一人格提示的实调用，尚未拿到线上用户回合样本。
- **仍未做**：`first_token_timeout` 死配置（接线或删除，属 LLM 网关 owner）；同步路径 `_sync_client` 仍固定 60s（一致性/ASE 侧挂死不在本闸门覆盖内）；provider 链结构性单源；记忆清洗是否有意与历史可恢复性。**新增限制（上线后明确）**：闸门只收紧了链首——agnes 20s + zhipu 25s（未配 `request_timeout`）= 45s > 编排层 30s 外层，**两家同时挂死时兜底句仍会出现**，靠 P1-2 熔断（连续 2 次失败拉黑 60s）兜住；后续按供应商逐家配 `request_timeout`（链上累计 ≤ 25s）才算彻底。
