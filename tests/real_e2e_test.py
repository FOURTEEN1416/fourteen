...
for i in range(15):
    time.sleep(1)
    try:
        urllib.request.urlopen("http://127.0.0.1:18080/api/shisi/characters", timeout=2)
        print(f"  ✅ 服务器就绪 ({i+1}s)")
        break
    except Exception:
        if i == 14:
...
if chars:
    ...
    status, body = api("GET", f"/api/shisi/characters/{urllib.parse.quote(cid, safe='')}")
...
status, body = api("POST", f"/api/shisi/emotion-stage/{test_cid}/evaluate?affinity=10")
...
results2 = []
for i in range(20):
    s1, _ = api("POST", f"/api/shisi/affinity/concurrent_{i}/update", {"delta": 1, "reason": "concurrent"})
    s2, _ = api("GET", f"/api/shisi/affinity/concurrent_{i}")
