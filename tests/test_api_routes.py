...
@pytest.mark.parametrize(
    ...
    [
        (misc_routes, 14, "stats/memory/diary/dates/logs/config/channels/routes/user-llm-config"),
...
def test_total_contribution_is_78(app):
    ...
    2026-08-28: training/extract 移除（9→8）；proactive +config(GET)/send/pause 手动控制（8→11）；misc +diary/important-dates×2（11→14）
    """
    modules = [
    ...
    assert total == 78, f"8 sub-routers contribute {total} routes, expected 78"
